...
try {
  await objStore.get(`${route.query.id}`)
  if (objStore.obj) {
    obj.value = objStore.obj
  } else {
    console.error("Object Empty Broski");
  }
} catch(e) {
...
<template>
  <div>
    <div class="page-header">
      <h3 class="text-2xl font-normal">{{ $t('labels.productUpdate') }}</h3>
      <span class="block text-slate-400 font-light">{{
        $t('features.vendors.productUpdateDesc')
      }}</span>
    </div>
          ...
          <template #content>
            <div>
              ...
              <div class="flex justify-end pt-6 gap-4">
                ...
                <PrimeButton type="submit" :loading="loading" :label="$t('labels.update')" />
              </div>
            </div>
          </template>
        </PrimeCard>
